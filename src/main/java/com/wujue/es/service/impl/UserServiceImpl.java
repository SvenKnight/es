package com.wujue.es.service.impl;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.wujue.es.dao.UserDao;
import com.wujue.es.dao.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper,User> implements UserService {
    ...
    @Override
    public void saveEs(List<User> list) {
        list = new ArrayList<>();
        ...
        for (int i = 0; i < 100; i++) {
            ...
            User user = new User(i + "", names[i1], descArr[i2],"110","110@qq.com",List.of(new Hobby("1",names[i1],descArr[i2])));
    ...
    @Override
    public void saveEs(User user) {
        Random random = new Random();
        int i1 = random.nextInt(names.length);
        int i2 = random.nextInt(descArr.length);
        user = new User(UUID.randomUUID().toString(), names[i1], descArr[i2],"110","110@qq.com",List.of(new Hobby("1",names[i1],descArr[i2])));
        userDao.save(user);

    }

    @Override
    public List<User> findUserByNameAndId(String name, String id) {
        TermQueryBuilder termQueryBuilder = QueryBuilders.termQuery("hobbyList.name", name);
        NestedQueryBuilder nestedQueryBuilder = QueryBuilders.nestedQuery("hobbyList", termQueryBuilder, ScoreMode.None);
        boolQueryBuilder.must(nestedQueryBuilder);

        queryBuilder.withQuery(boolQueryBuilder);
        Page<User> page = userDao.search(queryBuilder.build());
    ...
    @Override
    public void saveMysql(List<User> list) {
        list = new ArrayList<>();
...
//        userDao.save(user);
    ...
    @Override
    public List<User> findUserByNameAndIdMysql(String name, String id) {
        Hobby hobby = new Hobby();
        hobby.setName(name);
        return this.baseMapper.selectList(new QueryWrapper<User>().lambda()
                .apply("JSON_CONTAINS(hobby_list,JSON_OBJECT('name', {0}))", name));
    }
